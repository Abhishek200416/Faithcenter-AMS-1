        ...
        if (loc.scheduleType === 'once') {
            ...
            jobs.push(
                schedule.scheduleJob(
                    new Date(baseDate.getTime() - loc.earlyWindow * 60 _000),
            ...
            jobs.push(
                schedule.scheduleJob(
                    new Date(baseDate.getTime() + (loc.duration + loc.lateWindow) * 60 _000),
                    async() => {
            ...
            // weekly
            const dow = loc.daysOfWeek.map(d => ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].indexOf(d.toLowerCase()));
            const baseRule = { dayOfWeek: dow, hour: h };

            // early
            jobs.push(
                schedule.scheduleJob({...baseRule, minute: m - loc.earlyWindow },
                    () => notify('early', loc.earlyMsg)
                )
            );
            // on-time
            jobs.push(
                schedule.scheduleJob({...baseRule, minute: m },
                    () => notify('on-time', loc.onTimeMsg)
                )
            );
            // late + absent
            jobs.push(
                schedule.scheduleJob({...baseRule, minute: m + loc.duration + loc.lateWindow },
                    async() => {
                        await notify('late', loc.lateMsg);
                        await markAbsent(loc);
                    }
    ...
    const cancelLocation = async(req, res) => {
        ...
        await Attendance.destroy({
            where: { timestamp: {
                    [Op.between]: [loc.startAt, loc.expiresAt] } },
        })
        await loc.destroy()
